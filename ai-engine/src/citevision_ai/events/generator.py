...
class EventGenerator:
        ...
        for track in tracks:
            ...
            for rule in rules:
                ...
                if rule_type == "zone" and "zone" in rule:
                    events.extend(
                        self._check_zone(camera_id, track_id, cx, cy, rule, ts)
                    )
                elif rule_type == "line" and "line" in rule and prev:
                    events.extend(
                        self._check_line(camera_id, track_id, prev, (cx, cy), rule, ts)
                    )
                elif rule_type == "loitering" and "loitering" in rule:
                    events.extend(
                        self._check_loitering(camera_id, track_id, cx, cy, rule, ts)
                    )
    ...
    def _check_zone(
        ...
        if inside and zone_id not in prev_zones:
            ...
            events.append(
                self._make_event(camera_id, "zone_enter", ts, track_id, zone_id=zone_id)
            )
        elif not inside and zone_id in prev_zones:
            prev_zones.discard(zone_id)
            events.append(
                self._make_event(camera_id, "zone_exit", ts, track_id, zone_id=zone_id)
            )
        return events
