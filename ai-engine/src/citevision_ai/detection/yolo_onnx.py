...
class YoloOnnxDetector:
    ...
    def load(self) -> None:
        if not self.model_path.exists():
            logger.warning("ONNX model not found at %s; inference will return empty", self.model_path)
            return
        try:
            ...
            logger.info("Loaded YOLO ONNX model from %s", self.model_path)
        except Exception:
            logger.exception("Failed to load ONNX model; running in stub mode")
            self._session = None
    ...
    def _nms(self, boxes: list[dict]) -> list[dict]:
        ...
        while boxes:
            ...
            boxes = [
                b
                for b in boxes
                if self._iou(best["bbox"], b["bbox"]) < self.iou_threshold
            ]
        return kept
