...
func (s *Service) Append(ctx context.Context, req LogRequest) (*models.AuditEntry, error) {
	var prevHash string
	err := s.pool.QueryRow(ctx, `SELECT COALESCE(entry_hash, '') FROM audit_log ORDER BY id DESC LIMIT 1`).Scan(&prevHash)
	if err != nil {
		prevHash = ""
	}
	...
	err = s.pool.QueryRow(ctx, `
		INSERT INTO audit_log (org_id, user_id, action, resource_type, resource_id, ip_address, user_agent, payload, prev_hash, entry_hash, created_at)
...
func (s *Service) computeHash(prevHash string, req LogRequest, payload []byte, ts time.Time) string {
	...
	data := fmt.Sprintf("%s|%s|%s|%s|%s|%s",
		prevHash, req.Action, req.ResourceType, string(payload), ts.Format(time.RFC3339Nano),
		func() string {
			if req.UserID != nil {
...
func (s *Service) VerifyChain(ctx context.Context, limit int) (bool, error) {
	...
	for rows.Next() {
		...
		expected := s.computeHash(entry.PrevHash, LogRequest{
			UserID:       userID,
			Action:       entry.Action,
			ResourceType: entry.ResourceType,
			Payload:      map[string]interface{}{},
		}, entry.Payload, entry.CreatedAt)
		if expected != entry.EntryHash {
			// Recompute with stored payload for verification
			mac := hmac.New(sha256.New, s.signKey)
			uid := ""
			if userID != nil {
				uid = userID.String()
...
func (s *Service) List(ctx context.Context, orgID uuid.UUID, limit, offset int) ([]models.AuditEntry, error) {
	rows, err := s.pool.Query(ctx, `
		...
		FROM audit_log WHERE org_id = $1 ORDER BY id DESC LIMIT $2 OFFSET $3`, orgID, limit, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
