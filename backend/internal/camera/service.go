...
func (s *Service) CountByOrg(ctx context.Context, orgID uuid.UUID) (int, error) {
	var n int
	err := s.pool.QueryRow(ctx, `SELECT COUNT(*) FROM cameras WHERE org_id = $1 AND is_active = TRUE`, orgID).Scan(&n)
	return n, err
}
