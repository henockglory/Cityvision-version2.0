...
# re-sync after latest IoU soften
python3 - <<'PY'
...
assert "skip demo vehicle fallback for red_light" not in t
assert "Tighter IoU for red-light" not in t
assert "ship partial" in t
d.write_text(t, encoding="utf-8", newline="\n")
print("evidence_like_speed_ok")
PY
...
curl -sf --max-time 3 http://127.0.0.1:8001/health >/dev/null || { echo AI_FAIL; tail -20 logs/ai-engine.log; exit 1; }
