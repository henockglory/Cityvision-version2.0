...
import {
  ...
  ZoomIn, ZoomOut, Camera, Maximize2,
} from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3">
          <div className="cv-card overflow-hidden">
            <VideoPlaceholder label={selected.name} className="aspect-video rounded-none" />
        ...
        <div className="space-y-4">
              ...
              {cameras.map((cam) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                    activeId === cam.id ? 'bg-cv-accent/10 text-cv-accent border border-cv-accent/20' : 'hover:bg-cv-accent/5 text-cv-muted'
                  }`}
                >
                  {cam.name}
                </button>
          ...
          <div className="cv-card p-4">
            ...
            <div className="flex justify-center gap-2 mt-3">
              {ptzButtons.slice(4).map((btn) => (
                <button key={btn.label} type="button" onClick={() => playClick()} className="cv-btn-ghost p-2">
