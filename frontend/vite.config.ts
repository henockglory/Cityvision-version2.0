...
const proxy = {
  ...
  '/ai-engine': {
    ...
    changeOrigin: true,
    rewrite: (path: string) => path.replace(/^\/ai-engine/, ''),
  },
  '/rules-engine': {
    target: 'http://localhost:8010',
    changeOrigin: true,
    rewrite: (path: string) => path.replace(/^\/rules-engine/, ''),
  },
  '/frigate': {
    target: process.env.VITE_FRIGATE_URL || 'http://127.0.0.1:5000',
    changeOrigin: true,
    ws: true,
    rewrite: (path: string) => path.replace(/^\/frigate/, ''),
  },
  '/frigate-go2rtc': {
    ...
    changeOrigin: true,
    ws: true,
    rewrite: (path: string) => path.replace(/^\/frigate-go2rtc/, '/api/go2rtc'),
  },
};
...
export default defineConfig({
  ...
  // Product Start prefers scripts/serve-frontend-static.mjs; preview is a lighter Vite fallback.
  preview: {
    host: '0.0.0.0',
    port: 5174,
    proxy,
